				...
				<div class="list-option">
					<div class="list-option-inner">
						<select class="form-control" onchange="if(this.value) location.href=(this.value);">
							<option value="/store/listSearch" selected>기본 정렬순</option>
							<option value="/store/listStarSort">별점순</option>
							<option value="/store/listReviewSort" >리뷰 많은순</option>
						</select>
						<i class="arr-down"></i>
					</div>
				</div>
										...
										<tr onclick="location.href='/store/storeDetail?storeNo=${list.storeNo}'">
											...
											<td>
												<div class="restaurants-info" >
													...
													<ul>
														<li class="payment-methods yogiseo-payment">신용카드, 현금, ${list.storePaymentMethod}</li>
		...
		<div class="page_container">
		  <ul>
		    <c:if test="${pageMaker.prev}">
		    	<li class="paging"><a class="btn-paging" href="/store/listSearch${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
		    </c:if> 
		
		    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
		    	<li class="paging"><a class="btn-paging" href="/store/listSearch${pageMaker.makeQuery(idx)}">${idx}</a></li>
		    </c:forEach>
		
		    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
		    	<li class="paging"><a class="btn-paging" href="/store/listSearch${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
		    </c:if> 
		  </ul>
		</div>
	</div>
