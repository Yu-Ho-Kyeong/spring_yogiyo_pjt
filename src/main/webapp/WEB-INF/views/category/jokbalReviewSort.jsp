...
<!-- listAll----------------------------------------------------------------------------------------------------------->
	<div id="content" class="container-fluid">
		<div class="content">
				...
				<div class="list-option">
					<div class="list-option-inner">
						<select class="form-control" onchange="if(this.value) location.href=(this.value);">
							<option value="/category/jokbal">기본 정렬순</option>
							<option value="/category/jokbalStarSort">별점순</option>
							<option value="/category/jokbalReviewSort" selected>리뷰 많은순</option>
			...
			<div>
				<div>
					...
					<div class="restaurant-list">
						<c:forEach items="${listJokbalReviewSort}" var="store">
						<div class="col-sm-6">
			...
			<c:if test="${empty listJokbalReviewSort}">
		...
		<div class="page_container">
		  <ul>
		    <c:if test="${pageMaker.prev}">
		    	<li class="paging"><a class="btn-paging" href="/category/jokbal${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
		    </c:if> 
		
		    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
		    	<li class="paging"><a class="btn-paging" href="/category/jokbal${pageMaker.makeQuery(idx)}">${idx}</a></li>
		    </c:forEach>
		
		    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
		    	<li class="paging"><a class="btn-paging" href="/category/jokbal${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
		    </c:if> 
		  </ul>
		</div>
	</div>
